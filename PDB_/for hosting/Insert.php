...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(empty(trim($_POST["gender"]))){
        $gender_err = "Please enter username.";
    } else{
  ...
  /*if(empty(trim($_POST["batch"]))){
        $branch_err = "Please enter username."; $branch = 
    } else{
        $branch = trim($_POST["batch"]);
    }*/
  ...
  if(empty(trim($_POST["cgpa"]))){
    ...
    //echo $execval;
    echo "<h5>Registration successfully...</h5>";
    header("Location:/PDB_/Insert.php");
    $stmt->close();
    $conn->close();
